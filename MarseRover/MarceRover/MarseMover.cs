            ...
            while (true)
            {
                ...
                switch (option)
                {
                    ...
                    case (int)Option.AddRoverIntoArea:
                        ...
                        int plateauIndex = Convert.ToInt16(Console.ReadLine());

                        Console.WriteLine("Select Rover");

                        foreach (var rover in listOfRover)
                        {
                            Console.WriteLine("Press {0} To select Rover of id : {1}", rover.Key, rover.Value.Id);
                        }
                        
                        int roverInx = Convert.ToInt16(Console.ReadLine());

                        bool isAnyRoverInArea = listOfPlataeu[plateauIndex].IsAnyRoverInThisPoint(new int[] { listOfRover[roverInx].XPoint, listOfRover[roverInx].YPoint, listOfRover[roverInx].Id }, listOfPlataeu[plateauIndex]);

                        if (!isAnyRoverInArea)
                        {
                            int index = listOfPlataeu[plateauIndex].AddRoverIntoPlateau(listOfRover[roverInx]);
                    ...
                    case (int)Option.GiveCommand:
                        ...
                        foreach (var item in cmmd)
                        {
                            ...
                            if (item == (char)Action.Move)
                            {
                                ...
                                if (plataeu.IsNotBoundaryPoint(nextPoint, listOfPlataeu[plateauInx]) && !plataeu.IsAnyRoverInThisPoint(rover, listOfPlataeu[plateauInx]))
                                {
                                    rover.XPoint = nextPoint[0];
                                    rover.YPoint = nextPoint[1];
                                }
                                else
                                {
                            ...
                            {
                                rover.DirectionKey = rover.GetNextDirection(rover.DirectionKey, item);
                            }
                            else
                            {
